...
from synaesthesia.base_sensors.csv_dataset import CsvDataset
import numpy as np
...
def test_custom_concat_dataset_length(datasets, custom_concat_dataset):
    ...
    assert len(custom_concat_dataset) == expected_length, f"Expected length {expected_length}, but got {len(custom_concat_dataset)}"
...
def test_custom_concat_dataset_get_data(datasets, custom_concat_dataset):
    ...
    assert np.array_equal(actual_data['random_integer1'], expected_data['random_integer1']), "Mismatch in random_integer1"
    assert np.isclose(actual_data['index_1.5'], expected_data['index_1.5']), "Mismatch in index_1.5"
...
def test_custom_concat_dataset_get_timestamp(datasets, custom_concat_dataset):
    ...
    index_1 = 5
    expected_timestamp = datasets[0].get_timestamp(index_1)  # Get timestamp from first dataset
    actual_timestamp = custom_concat_dataset.get_timestamp(index_1)
    assert actual_timestamp == expected_timestamp, f"Timestamp mismatch at index 5: expected {expected_timestamp}, got {actual_timestamp}"

    index_2 = 2
    expected_timestamp = datasets[1].get_timestamp(2)  # Get timestamp from second dataset
    actual_timestamp = custom_concat_dataset.get_timestamp(len(datasets[0]) + index_2)  # Corresponding index
    assert actual_timestamp == expected_timestamp, f"Timestamp mismatch at index 32: expected {expected_timestamp}, got {actual_timestamp}"
